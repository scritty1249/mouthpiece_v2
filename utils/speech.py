    ...
    *,
    ...
    if use_prompt:
        assert len(prompt_text) == len(
            prompt_tokens
        ), f"Prompt text ({len(prompt_text)}) and tokens ({len(prompt_tokens)}) must have the same length"
...
def load_model(config_name, checkpoint_path, device="cuda"):
    ...
    with initialize(version_base="1.3", config_path="../lib/fish_speech/fish_speech/configs"):
...
class Model:
    def __init__(
        self,
        prompt_text: str,
        prompt_token: np.typing.NDArray,
        checkpoint_dir: Path = Path("checkpoints/openaudio-s1-mini"),
        ...
        self.prompt_tokens_list = None
        if self.prompt_token is not None:
            self.prompt_tokens_list = [torch.from_numpy(prompt_token)]
    ...
    @torch.no_grad()
    def generate_audio(self, compiled_tokens) -> tuple[np.typing.NDArray, int]:
        logger.info(f"Processing precomputed indices")
        indices = compiled_tokens.long()
        assert indices.ndim == 2, f"Expected 2D indices, got {indices.ndim}"
        indices_lens = torch.tensor([indices.shape[1]], device=self.device, dtype=torch.long)

        # Restore
        fake_audios, audio_lengths = self.audio_model.decode(indices, indices_lens)
        audio_time = fake_audios.shape[-1] / self.audio_model.sample_rate
